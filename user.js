...
module.exports = function createAuth(q, opts) {
  ...
  function verifyToken(req, res, next) {
    // 1. VERCEL İÇİN İSTİSNA (BYPASS) MANTIĞI
    const path = req.path || req.url;
    
    // Eğer istek bir statik dosyaysa (.js, .css, .png vb.) veya login sayfasıysa korumayı atla
    if (
        path === '/' || 
        path.includes('.') // .js, .css gibi dosyaları ayırt eder
    ) {
        return next();
    }
    ...
    try {
        ...
        // Cookie kontrolü
        if (!token && req.headers && req.headers.cookie && typeof req.headers.cookie === 'string') {
        ...
        // Token hala yoksa hata döndür
        if (!token) {
            return res.status(401).json({ error: 'missing authorization1' });
        }
