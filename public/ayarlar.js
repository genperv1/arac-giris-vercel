...
(function () {
  ...
  let pasifPage = 1;
  let pasifHasMore = false;
  let pasifDays = 180;
  let pasifRowsCache = [];
  let pasifSearchDebounce = null;
  const pasifSelectedIds = new Set();
  ...
  const SECTION_META = {
    'section-plaka': {
      title: 'Plaka geliş istatistikleri',
      desc: 'Takip formu yazdırıldığında plaka, firma ve ürün eşleşmeleri burada birikir. Satıra tıklayınca detay açılır.',
      hash: 'plaka',
    },
    'section-pasif': {
      title: 'Uzun süredir pasif şoförler',
      desc: 'Seçilen süredir hiç yazdırma yapmamış şoför kartlarını görüntüleyin ve temizleyin.',
      hash: 'pasif',
    },
    'section-eksik': {
      title: 'Eksik şoför bilgileri',
      desc: 'TC kimlik veya telefon eksik kayıtlar. Düzenle ile ana sayfada form açılır.',
      hash: 'eksik',
    },
    'section-ozmal': {
    ...
    'section-imza': {
      title: 'İmza ayarları',
      desc: 'Kantar ve sevkiyat saha imzalarını yönetin.',
      hash: 'imza',
    },
    'section-yazdir': {
      title: 'Yazdırma ayarları',
      desc: 'Takip formu düzeni, şablon ve yazdırma geçmişi temizliği.',
      hash: 'yazdir',
    },
    'section-ban': {
      title: 'IP engelleri',
    ...
    'section-yedek': {
      title: 'Yedekleme',
      desc: 'Tüm sistem verisini yedekleyin veya geri yükleyin.',
      hash: 'yedek',
    },
  };
  ...
  let sessionAuthWarned = false;

  function warnSessionRequired() {
    if (sessionAuthWarned) return;
    sessionAuthWarned = true;
    toast('Veri için ana sayfada (GIRIS) oturum açmanız gerekir. Ayar parolası (543723) tek başına yetmez.', true);
    showConnectionBanner('Oturum gerekli — önce ana sayfadan giriş yapın, sonra Ayarlar\'ı tekrar açın.');
  }

  function showConnectionBanner(msg) {
    let el = document.getElementById('ayConnBanner');
    if (!el) {
      el = document.createElement('div');
      el.id = 'ayConnBanner';
      el.style.cssText = 'margin:0 0 1rem;padding:.75rem 1rem;border-radius:10px;background:#fef2f2;border:1px solid #fecaca;color:#991b1b;font-size:.875rem;font-weight:600';
  ...
  async function fetchWithTimeout(url, opts, ms) {
    ...
    try {
      ...
      if (e && e.name === 'AbortError') {
        throw new Error('Sunucu yanıt vermedi — npm start çalışıyor mu? (http://localhost:3000)');
      }
      throw e;
    } finally {
      clearTimeout(timer);
  ...
  async function apiFetch(url, opts, retry) {
    const reqOpts = Object.assign({ credentials: 'include', headers: authHeaders(!!(opts && opts.body)) }, opts || {});
    let r;
    try {
      r = await fetchWithTimeout(url, reqOpts);
    } catch (e) {
      showConnectionBanner(String(e.message || 'Sunucuya bağlanılamadı — npm start ile sunucuyu başlatın.'));
      throw e;
    }
    const path = String(url || '');
    if (r.status === 401 && !path.includes('/settings/')) {
      if (retry !== false) {
  ...
  function showSection(id, opts) {
    ...
    if (!el || !AY_SECTIONS.includes(id)) return;
    activeSection = id;

    AY_SECTIONS.forEach((sid) => {
      const section = document.getElementById(sid);
      if (section) section.hidden = sid !== id;
    });

    document.querySelectorAll('.ay-nav-btn[data-goto]').forEach((btn) => {
      btn.classList.toggle('is-active', btn.getAttribute('data-goto') === id);
    });

    const meta = SECTION_META[id] || {};
    const titleEl = document.getElementById('ayActiveSectionTitle');
    const descEl = document.getElementById('ayActiveSectionDesc');
    if (titleEl) titleEl.textContent = meta.title || 'Ayarlar';
    if (descEl) descEl.textContent = meta.desc || '';

    if (opts.updateHash !== false && meta.hash) {
      try { history.replaceState(null, '', '#' + meta.hash); } catch (e) { /* ignore */ }
    }

    if (opts.load !== false) runSectionLoader(id);
  ...
  async function ensureSession() {
    try {
      let r = await fetchWithTimeout('/api/me', { credentials: 'include', headers: authHeaders(false) });
      if (r.status === 401) {
        try { localStorage.removeItem('authToken'); } catch (e) { /* ignore */ }
        r = await fetchWithTimeout('/api/me', { credentials: 'include', headers: authHeaders(false) });
      }
      if (r.ok) {
      ...
      console.warn('ensureSession /api/me', e);
      showConnectionBanner(String(e.message || 'Sunucuya bağlanılamadı — terminalde npm start çalıştırın.'));
    }
  ...
  async function loadPasifSummary() {
    try {
      const r = await apiFetch('/api/plaka-stats/summary?days=' + encodeURIComponent(String(pasifDays)));
      if (!r.ok) return;
      const d = await r.json();
      const el = document.getElementById('sumIdleDrivers');
      if (el) el.textContent = String(d.idleDriverCount ?? '—');
    } catch (e) {
      console.warn('pasif summary', e);
    }
  }
  ...
  async function loadSummary() {
    try {
      ...
      if (!r.ok) {
        if (r.status === 401) warnSessionRequired();
        return;
      }
      const d = await r.json();
      const set = (id, val) => {
      ...
      set('sumIdleDrivers', d.idleDriverCount);
    } catch (e) {
      console.warn('summary load', e);
      const tbody = document.getElementById('plakaTbody');
      if (tbody && tbody.textContent.indexOf('Yükleniyor') >= 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="ay-empty" style="color:#dc2626">Özet yüklenemedi — sunucu çalışıyor mu?</td></tr>';
      }
    }
  }
  ...
  function updatePasifPagerUi() {
    const info = document.getElementById('pasifPagerInfo');
    const prev = document.getElementById('pasifPrev');
    const next = document.getElementById('pasifNext');
    if (info) info.textContent = 'Sayfa ' + pasifPage + ' · sayfa başına ' + PAGE_SIZE + ' kayıt';
    if (prev) prev.disabled = pasifPage <= 1;
    if (next) next.disabled = !pasifHasMore;
  }

  function updatePasifToolbarUi() {
    const selectedInfo = document.getElementById('pasifSelectedInfo');
    const totalInfo = document.getElementById('pasifTotalInfo');
    if (selectedInfo) selectedInfo.textContent = pasifSelectedIds.size + ' seçili';
    if (totalInfo) {
      totalInfo.textContent = pasifRowsCache.length
        ? 'Bu sayfada ' + pasifRowsCache.length + ' kayıt'
        : 'Kayıt yok';
    }

    const pageIds = pasifRowsCache.map((r) => String(r.id));
    const allPageSelected = pageIds.length > 0 && pageIds.every((id) => pasifSelectedIds.has(id));
    const pageCb = document.getElementById('pasifSelectPage');
    const headCb = document.getElementById('pasifSelectAllHead');
    if (pageCb) pageCb.checked = allPageSelected;
    if (headCb) headCb.checked = allPageSelected;
  }

  function syncPasifRowChecks() {
    document.querySelectorAll('#pasifTbody tr[data-pasif-id]').forEach((tr) => {
      const id = tr.getAttribute('data-pasif-id');
      const cb = tr.querySelector('.pasif-row-check');
      if (cb) cb.checked = pasifSelectedIds.has(id);
    });
    updatePasifToolbarUi();
  }

  async function loadPasifDriversTable() {
    const tbody = document.getElementById('pasifTbody');
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="7" class="ay-empty">Yükleniyor…</td></tr>';
    loadPasifSummary();
    const q = (document.getElementById('pasifSearch')?.value || '').trim();
    const offset = (pasifPage - 1) * PAGE_SIZE;
    try {
      ...
      const data = await r.json();
      const items = data.items || [];
      pasifRowsCache = items;
      pasifHasMore = items.length >= PAGE_SIZE;

      if (!items.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="ay-empty">Seçilen sürede pasif şoför kaydı bulunamadı.</td></tr>';
        updatePasifPagerUi();
        updatePasifToolbarUi();
        return;
      }
      ...
      const rowNumStart = offset + 1;
      tbody.innerHTML = items.map((row, i) => {
        const checked = pasifSelectedIds.has(String(row.id));
        const phone = row.iletisim || '—';
        const kayit = row.kayitTarihi || fmtTs(row.kayitTs);
        return `<tr class="ay-row--inactive" data-pasif-id="${attrEsc(row.id)}">
          ...
          <td>
            <div class="ay-row-actions">
              <button type="button" class="ay-icon-btn ay-icon-btn--danger pasif-del-one" data-id="${attrEsc(row.id)}" title="Sil"><i class="fas fa-trash-alt" aria-hidden="true"></i></button>
            </div>
          </td>
        </tr>`;
      }).join('');

      tbody.querySelectorAll('.pasif-row-check').forEach((cb) => {
        cb.addEventListener('change', () => {
          const id = cb.getAttribute('data-id');
          if (!id) return;
          if (cb.checked) pasifSelectedIds.add(id);
          else pasifSelectedIds.delete(id);
          updatePasifToolbarUi();
        });
      });
      tbody.querySelectorAll('.pasif-del-one').forEach((btn) => {
      ...
      const headCb = document.getElementById('pasifSelectAllHead');
      if (headCb) {
        headCb.onclick = () => togglePasifPageSelection(headCb.checked);
      }
  ...
  function togglePasifPageSelection(checked) {
    pasifRowsCache.forEach((row) => {
      const id = String(row.id);
      if (checked) pasifSelectedIds.add(id);
      else pasifSelectedIds.delete(id);
    });
    syncPasifRowChecks();
  }

  async function deletePasifDrivers(ids) {
    const list = (ids || []).map((id) => String(id)).filter(Boolean);
    if (!list.length) {
      toast('Silinecek kayıt seçilmedi.', true);
      return;
    }
    try {
      const r = await apiFetch('/api/inactive-drivers/bulk-delete', {
        method: 'POST',
        body: JSON.stringify({ ids: list }),
      });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(data.error || 'Silinemedi');
      list.forEach((id) => pasifSelectedIds.delete(id));
      const deleted = Number(data.deleted) || 0;
      const skipped = Number(data.skipped) || 0;
      if (deleted > 0) {
        toast(deleted + ' pasif şoför kaydı silindi.' + (skipped ? ' (' + skipped + ' atlandı)' : ''));
      } else {
        toast('Kayıt silinemedi — yalnızca yazdırma yapmamış kartlar silinebilir.', true);
      }
      await loadSummary();
      await loadPasifSummary();
      await loadPasifDriversTable();
    } catch (e) {
      toast(String(e.message || 'Silme başarısız.'), true);
    }
  }

  async function handlePasifDeleteSelected() {
    const ids = [...pasifSelectedIds];
    if (!ids.length) {
      toast('Önce silmek istediğiniz kayıtları seçin.', true);
      return;
    }
    if (!(await confirm(ids.length + ' pasif şoför kaydı kalıcı olarak silinsin mi?'))) return;
    await deletePasifDrivers(ids);
  }

  async function handlePasifDeletePage() {
    const ids = pasifRowsCache.map((r) => String(r.id)).filter(Boolean);
    if (!ids.length) {
      toast('Bu sayfada silinecek kayıt yok.', true);
      return;
    }
    if (!(await confirm('Bu sayfadaki ' + ids.length + ' pasif şoför kaydı silinsin mi?'))) return;
    await deletePasifDrivers(ids);
  }
  ...
  function bindPasifUi() {
    document.getElementById('pasifIdleDays')?.addEventListener('change', (e) => {
      ...
      pasifPage = 1;
      loadPasifSummary();
      loadPasifDriversTable();
    });
    document.getElementById('pasifSearch')?.addEventListener('input', () => {
      clearTimeout(pasifSearchDebounce);
      pasifSearchDebounce = setTimeout(() => {
        pasifPage = 1;
        loadPasifDriversTable();
      }, 280);
    });
    document.getElementById('pasifRefreshBtn')?.addEventListener('click', () => {
      pasifPage = 1;
      loadPasifDriversTable();
    });
    document.getElementById('pasifSelectPage')?.addEventListener('change', (e) => {
      togglePasifPageSelection(!!e.target.checked);
    });
    document.getElementById('pasifClearSelBtn')?.addEventListener('click', () => {
      pasifSelectedIds.clear();
      syncPasifRowChecks();
    });
    document.getElementById('pasifDeleteSelBtn')?.addEventListener('click', handlePasifDeleteSelected);
    document.getElementById('pasifDeletePageBtn')?.addEventListener('click', handlePasifDeletePage);
    document.getElementById('pasifPrev')?.addEventListener('click', () => {
      if (pasifPage > 1) {
        pasifPage -= 1;
        loadPasifDriversTable();
      }
    });
    document.getElementById('pasifNext')?.addEventListener('click', () => {
      if (pasifHasMore) {
        pasifPage += 1;
        loadPasifDriversTable();
      }
  ...
  function setActiveTab(tab, opts) {
    ...
    const searchEl = document.getElementById('plakaSearch');
    if (searchEl) searchEl.placeholder = 'Plaka / firma / ürün ara…';
    updatePlakaThead(tab);
    if (opts.scroll !== false) showSection('section-plaka', { load: false, updateHash: false });
    loadPlakaTable();
  }
  ...
  function setupEmergencyBanOnly() {
    ...
    const banEl = document.getElementById('section-ban');
    if (banEl) banEl.hidden = false;
    document.querySelectorAll('.ay-nav-btn[data-goto]').forEach((btn) => {
      const goto = btn.getAttribute('data-goto');
      btn.style.display = goto === 'section-ban' ? '' : 'none';
    });
    const sidebar = document.querySelector('.ay-sidebar');
    if (sidebar) sidebar.style.display = '';
    showSection('section-ban', { updateHash: false, load: true });
    const desc = document.getElementById('ayActiveSectionDesc');
  ...
  function applyHashSection() {
    ...
    const map = {
      imza: 'section-imza',
      'section-imza': 'section-imza',
      plaka: 'section-plaka',
      'section-plaka': 'section-plaka',
      eksik: 'section-eksik',
      'section-eksik': 'section-eksik',
      pasif: 'section-pasif',
      ...
      'section-yazdir': 'section-yazdir',
    };
    const id = map[hash];
    if (id) setTimeout(() => showSection(id), 80);
  }
  ...
  function loadDashboardData() {
    loadMyIp().catch(function () { /* ignore */ });
    loadSummary().catch(function (e) {
      console.warn('summary', e);
      showConnectionBanner('Özet yüklenemedi — ana sayfada oturum açık mı?');
    });
    setActiveTab('products', { scroll: false });
    showSection('section-plaka', { load: false, updateHash: false });
    applyHashSection();
    if (window.SignatureRegistry) {
      window.SignatureRegistry.loadSignatures(true).catch(function () { /* ignore */ });
    }
  }
  ...
  async function init() {
    try {
      ...
      console.error('ayarlar init', e);
      const detail = e && e.message ? e.message : String(e);
      toast('Sayfa başlatılamadı: ' + detail, true);
      showConnectionBanner('Hata: ' + detail + ' — Ctrl+F5 ile yenileyin.');
    }
  }
