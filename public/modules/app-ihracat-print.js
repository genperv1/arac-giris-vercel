...
function _ihracatBindModalPlateAdd(modal, statusApi) {
  ...
  const onPlateCommit = async (row) => {
    ...
    if (plakaCell) {
      plakaCell.innerHTML = _ihracatPlakaCellHtml(plate, true, false, { excelSheet: true });
    }
    ...
    const storedKey = await _ihracatPersistPendingShipment({
      plate,
      rowKey: newKey,
    ...
    const addRowMarkup = _ihracatRenderExcelSheetAddRow();
...
async function showIhracatDetailsModal() {
  ...
  if (typeof repairIhracatRowFileNames === 'function') {
    ...
    if (repaired.changed && typeof saveDailyShipments === 'function') {
      saveDailyShipments(shipmentsRaw, meta).catch(() => {});
    }
  }
  ...
  const rowHtml = (s, status) => _ihracatRenderExcelSheetDataRow(s, { ...sheetCtx, status });

  const addRowHtml = () => _ihracatRenderExcelSheetAddRow();
  ...
  const renderTable = (items, title, badgeColor, tableCtx) => {
    ...
    const emptyHintHtml = isEmptyBlock && typeof _ihracatEmptyBlockHintRowHtml === 'function'
      ? _ihracatEmptyBlockHintRowHtml(sample)
      : '';
    return `
      <div style="margin-bottom:16px;">
        ...
        <table class="ihr-sevkiyat-table ihr-excel-sheet" style="width:100%;min-width:860px;max-width:100%;border-collapse:collapse;font-size:11px;margin-bottom:8px;table-layout:auto;">
          <colgroup>
            <col style="width:34px"><col style="width:118px"><col style="width:108px"><col style="width:44px"><col style="width:44px"><col style="width:44px"><col style="width:50px"><col style="width:62px"><col style="width:62px"><col style="width:54px"><col style="width:108px">
          </colgroup>
          <thead>
            ${_ihracatRenderExcelSheetTableHead()}
          </thead>
      ...
      <div style="
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:12px;flex-wrap:wrap;">
          <div style="flex:1;min-width:220px;">
            ...
            <p style="margin:0;font-size:12px;color:#64748b;">Satırda irsaliye, plaka, ambalaj ve net tonaj düzenlenir. <b>←</b> ok: boş çuval → çuval. Şoför yoksa <b>Kayıt Et</b> ile yeni araç kaydı açılır.</p>
          </div>
  ...
  const doSave = async () => {
    ...
    const ok = await _saveIhracatDetailsFromModal(freshRows, freshMeta);
    if (ok) {
