...
let __lastPrintTsByPlate = null;
let __lastPrintTsCacheKey = 0;

function _refreshLastPrintTsByPlate() {
  const cache = window.__ihracatRemotePrintCache;
  const key = String(cache?.ts || 0) + ':' + String(cache?.reports?.length || 0);
  if (__lastPrintTsByPlate && __lastPrintTsCacheKey === key) return __lastPrintTsByPlate;

  const map = new Map();
  try {
    const reports = (cache?.reports || []).filter((r) => r && String(r.type || '').toUpperCase() === 'PRINT');
  ...
  __lastPrintTsByPlate = map;
  __lastPrintTsCacheKey = key;
  return map;
}

function vehicleCardLastPrintTs(vehicle) {
  const snapTs = Number(vehicle?.lastPrintSnapshot?.ts || vehicle?.lastPrintSnapshot?.timestamp || 0);
  if (Number.isFinite(snapTs) && snapTs > 0) return snapTs;

  const plateKey = _normPlate(vehicle?.cekiciPlaka || '');
  if (!plateKey) return 0;

  return _refreshLastPrintTsByPlate().get(plateKey) || 0;
}

function vehicleCardLastPrintLabel(vehicle) {
  const ts = vehicleCardLastPrintTs(vehicle);
  if (!ts) return '';
  try {
...
function vehicleCardHTML(vehicle) {
  ...
  const footerStatusHtml = showBlockOverlay
    ...
    : '';
  const lastPrintTs = vehicleCardLastPrintTs(vehicle);
  const lastPrintLabel = lastPrintTs ? vehicleCardLastPrintLabel(vehicle) : '';
  const lastPrintIso = lastPrintTs ? new Date(lastPrintTs).toISOString() : '';
...
${overlay}
    ...
    <div class="vehicle-card__footer">
      ...
      <time class="vehicle-card__date"${lastPrintLabel ? ` datetime="${lastPrintIso}" title="Son takip formu yazdırma"` : ''}>${lastPrintLabel}</time>
      ${formBtn}
    </div>
  </div>`;
}
...
TC: ${vehicle.tcKimlik || ''}
Son takip formu: ${vehicleCardLastPrintLabel(vehicle) || '—'}`;
