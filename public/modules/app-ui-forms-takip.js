...
document.querySelectorAll('.eslestirme-duzenle-btn').forEach(btn => {
  btn.addEventListener('click', async function() {
    ...
    showEslestirmeModal();
        ...
        function syncVehicleFormFieldsFromState() {
            const fd = state.formData || {};
            const setVal = (id, val) => {
                const el = document.getElementById(id);
                if (el) el.value = val == null ? '' : String(val);
            };
            setVal('cekiciPlaka', fd.cekiciPlaka);
            setVal('dorsePlaka', fd.dorsePlaka);
            setVal('soforAdi', fd.soforAdi);
            setVal('soforSoyadi', fd.soforSoyadi);
            setVal('sofor2Adi', fd.sofor2Adi);
            setVal('sofor2Soyadi', fd.sofor2Soyadi);
            setVal('iletisim', fd.iletisim);
            setVal('tcKimlik', fd.tcKimlik);
            setVal('defaultFirma', fd.defaultFirma);
            setVal('defaultMalzeme', fd.defaultMalzeme);
            setVal('defaultSevkYeri', fd.defaultSevkYeri);
            setVal('defaultYuklemeNotu', fd.defaultYuklemeNotu);
        }
        ...
        function updateFormPanelPartial() {
            ...
            if (saveBtn) {
                saveBtn.innerHTML = `<i class="fas fa-save" aria-hidden="true"></i> ${state.editingId ? 'Güncelle' : 'Kaydet'}`;
            }
            if (state.showForm) syncVehicleFormFieldsFromState();
        }
