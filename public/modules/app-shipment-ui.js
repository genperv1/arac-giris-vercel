...
function buildIhracatLoadedRowBlockId(row, meta) {
  let fileName = '';
  try {
    const m = meta || (typeof loadDailyMeta === 'function' ? loadDailyMeta() || {} : {});
    if (typeof resolveIhracatRowFileLabel === 'function') {
      fileName = String(resolveIhracatRowFileLabel(row, m) || '').trim();
    }
  } catch (e) { /* ignore */ }
  if (!fileName) {
    ...
    fileName = (parts.length === 1 ? parts[0] : (parts[0] || '')).trim() || '_';
  }
