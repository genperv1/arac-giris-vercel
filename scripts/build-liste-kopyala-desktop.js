...
${logic}
  ...
  <script>
  (function () {
    var GATE_KEY = 'gpm_araclar_gate_v1';
    var GATE_PWD = '543723';
    ...
    function gateUnlocked() {
      try {
        ...
        return Number.isFinite(ts) && (Date.now() - ts < GATE_TTL);
      } catch (e) {
        return false;
      }
    }
    function gateUnlock() {
      try { sessionStorage.setItem(GATE_KEY, String(Date.now())); } catch (e) { /* ignore */ }
    }
    function ensureGate() {
      if (gateUnlocked()) return true;
      var pw = window.prompt('Liste kopyala şifresini girin:');
      if (pw == null || pw === '') return false;
      if (String(pw).trim() !== GATE_PWD) {
    ...
    function boot() {
      if (!ensureGate()) {
        document.body.innerHTML = '<p style="padding:2rem;font-family:Segoe UI,sans-serif">Şifre gerekli. Dosyayı yeniden açın.</p>';
        return;
      }
