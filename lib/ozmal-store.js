...
function migrateOzmalEntries(rawEntries) {
  ...
  let out = normalizeEntries(rawEntries).filter((entry) => normKey(entry.plaka) !== removedKey);
...
async function loadOzmalEntriesForLogin(q) {
  try {
    ...
    if (raw) {
      return stripPasswordFields(mergeWithSeedDefaults(JSON.parse(raw)));
    }
  } catch (e) { /* ignore */ }
  return stripPasswordFields(defaultEntries());
}
...
async function saveOzmalEntries(q, rawEntries) {
  ...
  const migrated = migrateOzmalEntries(rawEntries);
  const merged = [];
