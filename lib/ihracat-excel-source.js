...
async function readExcelFromStoredPath(source, opts) {
  const s = sanitizeSource(source);
  const needle = s.fileName ? s.fileName.toLowerCase() : '';
  const namedCandidates = [];
  if (await isReadableExcelFile(s.filePath)) {
    if (!needle || sanitizeFileName(s.filePath).toLowerCase() === needle) {
      namedCandidates.push(s.filePath);
    }
  }
  if (s.fileName) {
    const found = await findExcelByFileName(s.fileName, Object.assign({}, opts, { hintPath: s.filePath }));
    if (found) namedCandidates.push(found);
  }
  let filePath = await pickNewestExcel(namedCandidates);
  // İstenen ada uyan dosya yoksa: aynı klasördeki en yeni Excel (günlük dosya adı değişimi).
  // Çoklu Excel'de ada uyan dosya varken kardeş dosyaya kayma — bilinçli olarak yok.
