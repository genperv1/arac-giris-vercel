...
function localFileMeta(filePath) {
  const resolved = filePath || resolveLocalBgFilePath();
  if (!resolved) return null;
  try {
    const st = fs.statSync(resolved);
    const buf = fs.readFileSync(resolved, { encoding: null });
    if (!isValidImageBuffer(buf) || buf.length > MAX_IMAGE_BYTES) return null;
    return {
      kind: 'file',
      data: '',
      updatedAt: Math.round(st.mtimeMs),
      source: path.basename(resolved),
      contentType: mimeFromBuffer(buf),
      filePath: resolved,
    };
  } catch (_) {
    return null;
...
function isAcceptablePrintFormBg(record) {
  ...
  if (record.kind === 'file') return !!resolveLocalBgFilePath();
...
function writePrintFormBgMeta(q, meta) {
  if (!q) return Promise.resolve();
  const payload = {
    kind: 'file',
    updatedAt: Number(meta.updatedAt) || Date.now(),
    source: String(meta.source || 'file').slice(0, 40),
    contentType: String(meta.contentType || 'image/jpeg').slice(0, 40),
  };
  return q(
...
async function getPrintFormBgRecord(q) {
  ...
  const parsed = parseStoredValue(r.rows[0].value);
  if (parsed && parsed.kind === 'file') return parsed;
  return null;
}
...
async function setPrintFormBg(q, imageData, meta = {}) {
  ...
  await writePrintFormBgMeta(q, payload);
  return payload;
}
...
async function seedPrintFormBgIfEmpty(q) {
  ...
  return importLocalBgFile(q, filePath, { source: path.basename(filePath) });
}
