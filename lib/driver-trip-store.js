...
async function listTrips(q, { plaka, limit = 100 } = {}) {
  let sql = `SELECT * FROM driver_trips`;
  const params = [];
  if (plaka) {
    params.push(plaka);
    sql += ` WHERE plaka = $${params.length}`;
  }
...
function getKmMismatchWarning(trips) {
  ...
  const sorted = trips.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const warnings = [];
