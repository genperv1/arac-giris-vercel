...
function createPiyasaServerApi({ q, broadcastReportUpdate, sanitizeString, rootDir }) {
  ...
  function normalizePiyasaCustomersPayload(body) {
    ...
    const src = String(body.source || '').trim().toLowerCase();
    const allowEmpty = !!(body && (
    ...
    if (!customers.length && !allowEmpty) return null;
    return {
      version: 2,
      source: sanitizeString(String(body.source || (allowEmpty && !customers.length ? 'cleared' : 'manual')), 120) || 'manual',
      updatedAt: Date.now(),
      customers,
    };
  }
  ...
  async function seedPiyasaCustomersIfEmpty() {
    try {
      ...
      if (r.rows[0]) {
        try {
          const parsed = JSON.parse(r.rows[0].value);
          const count = Array.isArray(parsed?.customers) ? parsed.customers.length : 0;
          console.log(`ℹ️ Piyasa müşteri listesi mevcut (${count} kayıt)`);
        } catch (_) {
          console.log('ℹ️ Piyasa müşteri listesi kaydı mevcut');
        }
        return;
      }
