...
api.get("/plaka-stats/summary", async (req, res) => {
  try {
    const days = Math.min(Math.max(Number(req.query.days) || 60, 7), 365);
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    const vehPrintZero = `COALESCE(NULLIF(v.data::json->>'printCount','')::int, 0) = 0`;
    const [topR, onceR, neverR, idleR, totalPrintsR, productPairsR, editLogR, inactiveDriversR, ghostDriversR, idleDriversR] = await Promise.all([
      ...
      q(`SELECT COUNT(*)::int AS c FROM vehicles v WHERE v.cekiciplaka IS NOT NULL AND v.cekiciplaka <> '' AND ${vehPrintZero}`),
      q(`
        SELECT COUNT(*)::int AS c FROM vehicles v
        WHERE v.cekiciplaka IS NOT NULL AND v.cekiciplaka <> ''
        AND ${vehPrintZero}
        AND COALESCE(v.rejection_status, '') = ''
        AND NOT EXISTS (SELECT 1 FROM vehicle_edit_log el WHERE el.vehicle_id = v.id)
      `),
      q(`
        SELECT COUNT(*)::int AS c FROM vehicles v
        WHERE v.cekiciplaka IS NOT NULL AND v.cekiciplaka <> ''
    ...
    res.json({
      ...
      inactiveDriverCount: inactiveDriversR.rows[0]?.c || 0,
      ghostDriverCount: ghostDriversR.rows[0]?.c || 0,
      idleDriverCount: idleDriversR.rows[0]?.c || 0,
    });
  } catch (err) {
...
const VEH_PRINT_COUNT_SQL = `COALESCE(NULLIF(v.data::json->>'printCount','')::int, 0)`;
const VEH_PRINT_ZERO_SQL = `${VEH_PRINT_COUNT_SQL} = 0`;
const VEH_SOFOR_NAME_SQL = `TRIM(COALESCE(v.data::json->>'soforAdi','') || ' ' || COALESCE(v.data::json->>'soforSoyadi',''))`;

function mapInactiveDriverRow(row) {
  let data = {};
  try { data = JSON.parse(row.data || '{}'); } catch (e) { data = {}; }
  const soforAdi = String(data.soforAdi || '').trim();
  const soforSoyadi = String(data.soforSoyadi || '').trim();
  const soforName = [soforAdi, soforSoyadi].filter(Boolean).join(' ') || '—';
  return {
    id: row.id,
    plaka: row.plaka || data.cekiciPlaka || '',
    soforAdi,
    soforSoyadi,
    soforName,
    iletisim: String(data.iletisim || '').trim(),
    tcKimlik: String(data.tcKimlik || '').trim(),
    printCount: Number(data.printCount) || 0,
    kayitTs: row.sort_ts,
    kayitTarihi: data.kayitTarihi || null,
    hasEditLog: !!row.has_edit_log,
    rejected: !!row.rejected,
    idleKind: row.idle_kind || 'never_printed',
  };
}

api.get('/inactive-drivers', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT v.id,
             v.cekiciplaka AS plaka,
             v.data,
             v.sort_ts,
             COALESCE(v.rejection_status, '') <> '' AS rejected,
             EXISTS (SELECT 1 FROM vehicle_edit_log el WHERE el.vehicle_id = v.id) AS has_edit_log,
             CASE
               WHEN v.sort_ts < $1 THEN 'idle_long'::text
               ELSE 'never_printed'::text
             END AS idle_kind
      FROM vehicles v
      WHERE v.cekiciplaka IS NOT NULL AND v.cekiciplaka <> ''
      AND ${VEH_PRINT_ZERO_SQL}
    `;
    const params = [cutoff];

    if (tab === 'ghost') {
      sql += ` AND COALESCE(v.rejection_status, '') = '' AND NOT EXISTS (SELECT 1 FROM vehicle_edit_log el WHERE el.vehicle_id = v.id)`;
    } else if (tab === 'idle') {
      sql += ` AND v.sort_ts < $1`;
    }
    ...
    if (search) {
      ...
      const i = params.length;
      sql += ` AND (
        ${plateNormSql('v.cekiciplaka')} LIKE $${i - 2}
        OR UPPER(${VEH_SOFOR_NAME_SQL}) LIKE $${i - 1}
        OR UPPER(COALESCE(v.data::json->>'iletisim', '')) LIKE $${i}
      )`;
    }

    sql += ` ORDER BY v.sort_ts ASC, v.id ASC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
    params.push(limit, offset);
    ...
    res.json({
      tab,
      days,
      items: (r.rows || []).map(mapInactiveDriverRow),
      limit,
      offset,
    });
  } catch (err) {
    console.error('GET /inactive-drivers error', err);
    sendApiError(res, err, 500, 'INACTIVE_DRIVERS_FAILED');
  }
});

api.post('/inactive-drivers/bulk-delete', requireValidSession, async (req, res) => {
  try {
    const rawIds = Array.isArray(req.body?.ids) ? req.body.ids : [];
    const ids = [...new Set(rawIds.map((id) => sanitizeString(String(id || ''), 100)).filter(Boolean))].slice(0, 200);
    if (!ids.length) {
      return res.status(400).json({ ok: false, error: 'Silinecek kayıt seçilmedi' });
    }
    ...
    deleted.forEach((id) => {
      try { broadcastEvent('vehicle_deleted', { id }); } catch (e) { /* ignore */ }
    });
    res.json({
      ok: true,
    ...
    console.error('POST /inactive-drivers/bulk-delete error', err);
    sendApiError(res, err, 500, 'INACTIVE_DRIVERS_DELETE_FAILED');
  }
});
}
